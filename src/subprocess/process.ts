...
export class NeevSubprocessHandle implements SubprocessHandle {
   ...
   * @param pollMs - reserved for future status polling.
   */
  constructor(
  ...
  private async run(): Promise<SubprocessOutcome> {
    ...
    // A terminate() that raced process start is honored now that the id exists.
  ...
  /** Deliver SIGTERM, then SIGKILL after the spec's grace, to the process. */
